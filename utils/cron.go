...
func SetupAndRunCron(log *zap.Logger, config *Config) error {
	...
	for _, rule := range config.Rules {
		...
		_, cronErr := c.AddFunc(rule.Schedule, func() {
			...
			case "trading_to_cold_custody":
				...
				if err != nil {
					log.Error("failed to query hot wallet balances: %v", zap.Error(err))
				}
			...
			case "cold_custody_to_trading":
				...
				if err != nil {
					log.Error("Failed to query cold wallet balances: %v", zap.Error(err))
				}
